...
def mock_coordinator():
    with (
        patch("src.agents.agent_a.AgentA"),
        ...
        patch("src.core.agent_manager.AgentA"),
    ):  # Also patch where it is used in AgentManager
...
async def test_coordinator_chat_smoke(mock_coordinator):
    ...
    response = await mock_coordinator.chat_async(
        "Hello", {"tenant_id": "default", "username": "test", "role": "user"}
    )
...
def test_ingestion_pipeline_smoke(mock_coordinator):
    ...
    mock_coordinator.pipeline_manager.run_ingestion_pipeline.assert_called_once_with(
        "wash", False, None
    )
