...
| 语义记忆 | FAISS、BM25、CrossEncoder、SQLite 元数据与 MinIO 同步已形成混合 RAG | Phase 2 收敛为 PostgreSQL FTS + pgvector，保留 CrossEncoder 精排和引用 | ACL、版本、时间有效性、真实评测 |
...
- **M2 直接收敛检索底座**：PostgreSQL FTS + pgvector 替换 FAISS、BM25 与 SQLite；不保留双写或旧检索路径。
...
| 阶段 | 当前状态 | 用户结果 | 已验证门禁 | 下一步 |
| --- | --- | --- | --- | --- |
| M0 / Phase 0 | 已完成 | 系统不越权、不假成功，现状可测量 | 安全、租户、故障传播与评测基线 | 保持 CI 回归 |
| M1 / Phase 1 | 已完成 | 智能体能规划、审批、暂停与恢复 | 5/5 控制面任务与运行时回归 | 保持单智能体边界 |
| M2 / Phase 2 | 已完成 | 跨会话记忆可追踪、可删除 | PostgreSQL 唯一路径、20/20 记忆评测、跨 tenant 为 0 | 以治理策略维护 |
...
| 语义记忆 | 企业文档、事实、引用和向量 | PostgreSQL + pgvector + PostgreSQL 全文检索，CrossEncoder 精排 |
...
6. 直接以 PostgreSQL FTS + pgvector + RRF 替换 FAISS、BM25 与 SQLite RAG 元数据；保留 CrossEncoder 精排和引用。
...
- PostgreSQL 唯一路径的离线评测、真实任务、延迟、删除与恢复均达到预先记录的门禁；
- 只有通过写入门的内容进入长期记忆。
...
6. 以 PostgreSQL 作为文档和长期记忆的统一底座，不保留 FAISS/BM25/SQLite 检索路径。
...
| PostgreSQL + pgvector | 干净环境的质量、权限、删除和恢复门禁通过 | 不发布；不保留旧检索兼容路径 |
| RAG 存储收敛 | PostgreSQL FTS + pgvector + CrossEncoder 通过评测与真实任务 | 修复新路径，不恢复 FAISS/BM25/SQLite |
| Redis 长期数据迁出 | PostgreSQL 路径完成备份、删除和恢复验证 | Redis 仅保留 TTL 状态与缓存 |
