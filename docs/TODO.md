...
## 当前执行计划：工程候选版（2026-09-18 新增）

目标是交付“等待业务验收、工程证据完整”的候选版。以下阶段均未完成退出验收，不改变既有
...
- [ ] EC2：修复中英文抽取与拒答缺陷，统一回答状态、实际引用及版本化 verifier。
  - v2 契约及引用/反馈/前端已贯通；旧 PDF 两个正例回退，当前 NO_GO，优先修复且不改变原质量门槛。
- [ ] EC3：复用 H5 实现离线 LLM judge，完成已知答案检验、注入测试和三次冻结评测。
  - 已实现离线输出校验、统计与只读报告 verifier；实际 provider/预算/审计 runner 及三次评测尚未执行。
- [ ] EC4：冻结有效训练配置，打通批准/执行/产物核验，完成真实 GPU smoke。
- [ ] EC5：完成精确候选的集成、容量、部署和恢复验证，聚合证据并保留外部待验收项。

执行顺序 EC0 → EC1 → EC2 → EC3，EC4 可在 EC0 后独立推进；全部完成后进入 EC5。
