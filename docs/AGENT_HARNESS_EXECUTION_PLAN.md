...
**状态：** TVE-0--TVE-4、EL-1 与 EL-2 已完成当前门禁；EL-3 决策链已实现，真实终局为
`BLOCKED / candidate_unavailable`，EL-4 保持 `not-enabled`。EL-1 已把 `/api/chat` 绑定到
...
没有伪造 candidate arm，结论保持 `BLOCKED`。只有补充非 holdout、已许可的 train/validation 来源并
重跑 EL-2 后，才重启受控 A/B；在此之前不进入 EL-4。
