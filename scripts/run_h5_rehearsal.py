...
async def run_capture(
    runtime: AgentRuntime, identity: dict[str, str], scope: str, number: int
) -> dict[str, Any]:
...
async def run_job(
    ...
    *,
    ...
    if current["state"] != "succeeded":
        raise RuntimeError(
            f"rehearsal_job_failed:{current['state']}:{current.get('finish_reason')}"
        )
...
def main() -> None:
    ...
    tools.register(
        ToolSpec(
            ...
            handler=lambda arguments: {
                ...
                "observed_scope": [f"simulation:{tenant}:trial"],
            },
            schema={
                "type": "object",
                "required": ["trial"],
    ...
    for name, kind, schema, scope_for in (
        (
            ...
            "lora_train",
            {
                ...
                "properties": {"input_key": {"type": "string"}, "input_sha256": {"type": "string"}},
                "additionalProperties": False,
            },
            lambda arguments, _identity: [f"raw:{arguments['input_key']}"],
        ),
        (
            "h5_model_evaluate",
            "model_evaluate",
            {
                "type": "object",
                "required": ["input_key", "input_sha256", "evaluation_id"],
                "properties": {
            ...
            lambda arguments, _identity: [f"evaluation:{arguments['evaluation_id']}"],
        ),
    ):
        tools.register(
            ToolSpec(
    ...
    runtime = AgentRuntime(
        ...
        tools,
        verifiers,
        EvidenceService(database_url, evidence, tools.sensitivity),
        job_service,
    )
    ...
    for number in range(1, 4):
        ...
        evaluations.finish_trial(
            ...
            transcript_sha256=upload(store, transcript_key, transcript),
            simulation=True,
        )
    ...
    _, base_result = asyncio.run(
        run_job(
            runtime,
            owner,
            tool="h5_model_evaluate",
            input_key=base_key,
            input_sha256=base_hash,
            scope=f"evaluation:{base_evaluation}",
            extra={"evaluation_id": base_evaluation},
        )
    )
    ...
    candidate_hash = upload(
        store, candidate_key, json.dumps(candidate_input, sort_keys=True).encode()
    )
    _, candidate_result = asyncio.run(
        run_job(
            runtime,
            owner,
            tool="h5_model_evaluate",
            ...
            scope=f"evaluation:{candidate_evaluation}",
            extra={"evaluation_id": candidate_evaluation},
        )
    )
    ...
    report = {
        ...
        "snapshot_id": snapshot_id,
        "adapter_id": adapter_id,
        "rollback_release_id": rollback_release,
        "promoted_release_id": promoted_release,
        "limitations": [
            "synthetic data",
            "empty-substring evaluator assertion",
            "not H5 final evidence",
        ],
    }
    report_key = f"{prefix}/report.json"
    report["report_key"] = report_key
    report["report_sha256"] = upload(
        store, report_key, json.dumps(report, ensure_ascii=False, sort_keys=True).encode()
    )
